resource automation_workflow oobe_set_requires_tagging_on_upload {
    name = 'Foundation: Handle metadata on upload'
    description = ''
    content = 'trigger "Location State Changed" {
	type = "Location State Changed"
	resolves = "Set available for CV = No"
	new_location_state = "online"
	upload_computer = ""
	ignore_derived_assets = "true"
	ignore_replaced_assets = "true"
}

filter "Status empty?" {
	type = "Is Metafield Empty"
	asset_item_id = "@sourceAssetItemId"
	metafield = "guid:${to_string(data.combovalue_metafield.options_status.item_guid)}"
	negate = "false"
	needs = []
}

action "Set Status = Awaiting metadata" {
	type = "Set ComboValue Metafield"
	needs = "Status empty?"
	meta_field = "guid:${to_string(data.combovalue_metafield.options_status.item_guid)}"
	new_value = "${to_string(resource.combo_value.status_requires_tagging.option_value)}"
	asset_item_ids = "@sourceAssetItemId"
}

action "Set available for CV = No" {
	type = "Set ComboValue Metafield"
	needs = "Set Status = Awaiting metadata"
	meta_field = "guid:${to_string(resource.combovalue_metafield.available_for_content_viewers_50398.item_guid)}"
	new_value = "${to_string(resource.combo_value.available_for_content_viewers_no_50398.option_value)}"
	asset_item_ids = "@sourceAssetItemId"
}
'
    is_disabled = false
    tags = []
    system = false
}

