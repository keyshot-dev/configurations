...
resource configservice_label notification_panel_notification_panel_list_collection_notification {
  ...
  default_label_values = [
    {
      default_translation = '{actionType, select, 421 {<b>{{userName}}</b> has invited you to join a shared collection
      called "{{collectionName}}".}}'
      language_id = data.language.english.id
    },
    {
      default_translation = '{actionType, select, 421 {<b>{{userName}}</b> har inviteret dig til at deltage i en
      collection called "{{collectionName}}".}}'
      language_id = data.language.danish.id
    }
  ]
}
