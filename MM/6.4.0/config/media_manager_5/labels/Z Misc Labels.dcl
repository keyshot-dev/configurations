...
resource configservice_label aria_label_chip {
  ...
  default_label_values = [
    {
      default_translation = '{action, select, icon-plus {add} icon-x {remove} icon-arrow_drop_down {choose action} other {{{fallback}}}}'
      language_id = data.language.english.id
    },
    {
      default_translation = '{action, select, icon-plus {tilføj} icon-x {fjern} icon-arrow_drop_down {vælg handling} other {{{fallback}}}}'
      language_id = data.language.danish.id
    }
  ]
}

resource configservice_label aria_label_pages {
  key = 'ARIA_LABEL_PAGES'
  group = 'Z Misc Labels'
  product_id = resource.configservice_product.media_manager_5.id
  default_label_values = [
    {
      default_translation = 'Page {{page}}'
      language_id = data.language.english.id
    },
    {
      default_translation = 'Side {{page}}'
      language_id = data.language.danish.id
    }
  ]
}
