...
resource configservice_label administration_tools_system_asset_relation_types_changes_saved_error_body {
  ...
  default_label_values = [
    {
      default_translation = 'Asset relation type not saved.{error, select, undefined {} other {\n{error}}}'
      language_id = data.language.english.id
    },
    {
      default_translation = 'Asset-relationstypen blev ikke gemt.{error, select, undefined {} other {\n{error}}}'
      language_id = data.language.danish.id
    }
  ]
}
