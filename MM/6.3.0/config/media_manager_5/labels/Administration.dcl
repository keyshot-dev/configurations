...
resource configservice_label administration_tools_system_metadata_edit_save_error_body {
  ...
  default_label_values = [
    {
      default_translation = 'Changes not saved.{error, select, undefined {} other {{error}}}'
      language_id = data.language.english.id
    },
    {
      default_translation = 'Ændringerne blev ikke gemt.{error, select, undefined {} other {{error}}}'
      language_id = data.language.danish.id
    }
  ]
}
