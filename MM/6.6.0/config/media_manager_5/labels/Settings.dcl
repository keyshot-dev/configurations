...
resource configservice_label settings_logo_settings_configurator_description {
  ...
  default_label_values = [
    ...
    {
      default_translation = '{type, select, primary {Dette logo vises i øverste venstre hjørne af navigationslinjen.} secondary {Dette logo vises på loginskærmen.} secondaryInverted {Dette logo vises på loginskærmen, når mørk tilstand er aktiveret.}}'
      language_id = data.language.danish.id
    }
  ]
}
...
resource configservice_label selected_assets_settings_section_header_description {
  ...
  default_label_values = [
    {
      default_translation = 'The first {{primaryCount}} actions appear as primary buttons in the overlay; the rest are shown in the overflow menu.\n\nSome actions are only available in specific views - for example, Insert is only shown in the Embedded view (Connector).'
      language_id = data.language.english.id
    },
    {
      default_translation = 'De første {{primaryCount}} handlinger vises som primære knapper i overlayet; resten placeres i mere-menuen.\n\nNogle handlinger er kun tilgængelige i bestemte visninger – f.eks. vises Indsæt kun i den indlejrede visning (Connector).'
      language_id = data.language.danish.id
    }
  ]
}
