...
resource configservice_label administration_tools_api_keys_section_modal_copy_your_api_key {
  key = 'ADMINISTRATION_TOOLS_API_KEYS_SECTION_MODAL_COPY_YOUR_API_KEY'
  group = 'administration-tools - api-keys'
  product_id = resource.configservice_product.media_manager_5.id
  default_label_values = [
    {
      default_translation = 'Copy your API key'
      language_id = data.language.english.id
    },
    {
      default_translation = 'Kopiér din API-nøgle'
      language_id = data.language.danish.id
    }
  ]
}

resource configservice_label administration_tools_api_keys_section_modal_your_api_key_is {
  key = 'ADMINISTRATION_TOOLS_API_KEYS_SECTION_MODAL_YOUR_API_KEY_IS'
  group = 'administration-tools - api-keys'
  product_id = resource.configservice_product.media_manager_5.id
  default_label_values = [
    {
      default_translation = 'Your newly created API key is:'
      language_id = data.language.english.id
    },
    {
      default_translation = 'Din nyoprettede API-nøgle er:'
      language_id = data.language.danish.id
    }
  ]
}

resource configservice_label administration_tools_api_keys_section_modal_make_sure_copy_text {
  key = 'ADMINISTRATION_TOOLS_API_KEYS_SECTION_MODAL_MAKE_SURE_COPY_TEXT'
  group = 'administration-tools - api-keys'
  product_id = resource.configservice_product.media_manager_5.id
  default_label_values = [
    {
    ...
    {
      default_translation = 'Sørg for at kopiere din API-nøgle nu. Du vil ikke kunne se den igen'
      language_id = data.language.danish.id
    }
  ]
}
...
resource configservice_label administration_tools_dashboard_tool_clear_filters {
  key = 'ADMINISTRATION_TOOLS_DASHBOARD_TOOL_CLEAR_FILTERS'
  group = 'administration-tools'
  product_id = resource.configservice_product.media_manager_5.id
  default_label_values = [
    {
...
resource configservice_label administration_tools_system_metadata_restrict_to_asset_categories {
  ...
  default_label_values = [
    ...
    {
      default_translation = 'Vis på asset-kategorier'
      language_id = data.language.danish.id
    }
  ]
}
