...
resource configservice_label preview_add_comment_add_message {
  ...
  default_label_values = [
    {
      default_translation = '{mode, select, edit {Update comment} reply {Reply} add {Type a message}}'
      language_id = data.language.english.id
    },
    {
      default_translation = '{mode, select, edit {Opdatér kommentar} reply {Svar} add {Skriv en besked}}'
      language_id = data.language.danish.id
    }
  ]
}
