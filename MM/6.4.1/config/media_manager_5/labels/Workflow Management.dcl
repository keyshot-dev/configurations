...
resource configservice_label workflow_editor_workflow_asset_search_request_input_dialog_test_search {
  key = 'WORKFLOW_EDITOR_WORKFLOW_ASSET_SEARCH_REQUEST_INPUT_DIALOG_TEST_SEARCH'
  group = 'Workflow Management'
  product_id = resource.configservice_product.media_manager_5.id
  default_label_values = [
    {
      default_translation = 'Test search'
      language_id = data.language.english.id
    },
    {
      default_translation = 'Test søgning'
      language_id = data.language.danish.id
    }
  ]
}
...
resource configservice_label workflow_editor_workflow_asset_search_request_keyword_input_only_include_assets_with_a_value {
  ...
  default_label_values = [
    {
      default_translation = 'Only include items with a value'
      language_id = data.language.english.id
    },
    {
      default_translation = 'Inkluder kun items som har en værdi'
      language_id = data.language.danish.id
    }
  ]
}
...
resource configservice_label workflow_editor_workflow_asset_search_request_keyword_input_only_include_assets_without_a_value {
  ...
  default_label_values = [
    {
      default_translation = 'Only include items without a value'
      language_id = data.language.english.id
    },
    {
      default_translation = 'Inkluder kun items uden værdier'
      language_id = data.language.danish.id
    }
  ]
}
...
resource configservice_label transcode_rendition_status_no_qualitites {
  key = 'TRANSCODE_RENDITION_STATUS_NO_QUALITIES'
  group = 'Transcode rendition status - transcode-renditions-status'
  product_id = resource.configservice_product.media_manager_5.id
  default_label_values = [
    {
...
resource configservice_label transcode_rendition_status_state_table_row {
  key = 'TRANSCODE_RENDITION_STATUS_STATE_TABLE_ROW'
  group = 'Transcode rendition status - transcode-renditions-status'
  product_id = resource.configservice_product.media_manager_5.id
  default_label_values = [
    {
      default_translation = '{state, select, FilesFoundInBothStorageUnits {Both the file and the file reference exist} MissingInBlobStorage {File missing in the Blob storage} MissingInDatabase {File reference missing in database}}'
      language_id = data.language.english.id
    },
    {
      default_translation = '{state, select, FilesFoundInBothStorageUnits {Både filen og fil-referencen findes} MissingInBlobStorage {Filen mangler i Blob-storage} MissingInDatabase {Fil-referencen mangler i databasen}}'
      language_id = data.language.danish.id
    }
  ]
}
